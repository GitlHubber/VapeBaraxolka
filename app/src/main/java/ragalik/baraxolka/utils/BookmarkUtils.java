...
public class BookmarkUtils {
    ...
    private static BookmarkUtils instance;

    public static BookmarkUtils getInstance () {
        if (instance == null) {
            instance = new BookmarkUtils();
        }
        return instance;
    }

    public void setDeleteBookmark (int ad_id, String flag, Context context, Ad ad, TextView bookmarkText, AppCompatButton bookmarkButton) {
            ...
            public void onResponse(Call<ServerResponse> call, Response<ServerResponse> response) {
                if (response.body() != null && response.body().getSuccess() == 1) {
                    if (flag.equals("set")) {
                        ...
                        bookmarkText.setTextColor(ContextCompat.getColor(context, R.color.colorAccent));
                        bookmarkButton.setBackground(context.getDrawable(R.drawable.ic_baseline_bookmarks_24));
                        ad.setFavourite(true);
                    } else if (flag.equals("delete")) {
                        Toast.makeText(context, "Объявление убрано из закладок", Toast.LENGTH_SHORT).show();
                        //bookmark.setChecked(false);
                        bookmarkText.setTextColor(ContextCompat.getColor(context, R.color.colorSecondaryText));
                        bookmarkButton.setBackground(context.getDrawable(R.drawable.ic_baseline_bookmark_24));

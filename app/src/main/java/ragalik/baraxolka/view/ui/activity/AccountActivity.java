...
import ragalik.baraxolka.view.ui.fragment.EditAccountFragment;
...
//
        ...
        accountViewStub = findViewById(R.id.account_view_stub);
        accountViewStub.inflate();

        pnLayout = findViewById(R.id.account_pn_layout);
        regionLayout = findViewById(R.id.account_region_layout);
        reportsLayout = findViewById(R.id.account_reports_layout);
        accountFab = findViewById(R.id.account_fab);

        pnLayout.setOnClickListener(v -> {
//            getSupportFragmentManager().beginTransaction().
//                    addToBackStack(null).replace(R.id.accountCoordinator, new EditAccountFragment("phone")).commit();

            Intent intent = new Intent(this, EditAccountFragment.class);
            startActivity(intent);
        });

        regionLayout.setOnClickListener(v -> {
//            getSupportFragmentManager().beginTransaction().
//                    addToBackStack(null).replace(R.id.accountCoordinator, new EditAccountFragment("region")).commit();
            Intent intent = new Intent(this, EditAccountFragment.class);
            startActivity(intent);
        });

        signOut = findViewById(R.id.signOutButtonAccount);
...
//                        addToBackStack(null).replace(R.id.constrLayout, new AdsFragment()).commit();
        ...
        emailTV = findViewById(R.id.account_email);
        phoneNumberTV = findViewById(R.id.account_phone);
        accountPhoto = findViewById(R.id.profilePhoto);

        accountPNInfo = findViewById(R.id.tw_account_info);
        tw_hide_phone = findViewById(R.id.tw_hide_phone);
        hidePhone = findViewById(R.id.cb_showHidePN);
        hidePhoneLayout = findViewById(R.id.hide_phone_layout);
        townTV = findViewById(R.id.account_town);
    ...
    private void uploadFile() {
            ...
            apiClient.uploadAccountImage(body, RequestBody.create(MediaType.parse("multipart/form-data"), MainActivity.sp.getString("email", ""))).enqueue(new Callback<String>() {
                ...
                public void onResponse(Call<String> call, Response<String> response) {
                    pDialog.dismiss();
                    Toast.makeText(pDialog.getContext(), "Изображение загружено!", Toast.LENGTH_LONG).show();
...
//        mAdView.loadAd(adRequest);
        ...
        accountPhoto.setOnClickListener(v -> {
            if (!MainActivity.sp.getString("image", "").equals("null")) {
                ...
                getSupportFragmentManager().beginTransaction()
                        .replace(R.id.accountCoordinator, new FullImageLayout(0, arrayList)).addToBackStack("").commit();
            }
        });
        ...
        accountFab.setOnClickListener(v -> {
            ...
            accountImageMenu.show(getSupportFragmentManager(), "");
        });
        ...
        call.enqueue(new Callback<List<User>>() {
            ...
            public void onResponse(Call<List<User>> call, Response<List<User>> response) {
                if (!response.body().isEmpty()) {
                    ...
                    Toast.makeText(pDialog.getContext(), "Ошибка загрузки профиля", Toast.LENGTH_LONG).show();
            ...
            public void onFailure(Call<List<User>> call, Throwable t) {
                Toast.makeText(pDialog.getContext(), t.getLocalizedMessage(), Toast.LENGTH_LONG).show();
            }
        });
    }
}
